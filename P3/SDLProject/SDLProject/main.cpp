...
void ProcessInput() {
    ...
    if (keys[SDL_SCANCODE_LEFT]) {
        state.player->acceleration.x = -0.7f;
    }
...
void Render() {
    ...
    if (!state.player->isActive && (!state.platforms[2].isActive || !state.platforms[3].isActive || !state.platforms[4].isActive)){
